#!/bin/bash
# Simple script to run Ollama setup and product matching
# Copy and paste this into your SSH session

echo "🚀 Starting Ollama Setup and Product Matching..."
echo "=============================================="
...
if [ ! -f "setup_ollama_complete.sh" ]; then
    echo "📝 Creating setup script..."
    cat > setup_ollama_complete.sh << 'SETUP_EOF'
#!/bin/bash
...
sudo apt install -y python3 python3-pip python3-venv git curl wget unzip build-essential software-properties-common apt-transport-https ca-certificates gnupg lsb-release htop tree
...
cat > /home/ubuntu/matchfuzzy_ollama.py << 'PYTHON_EOF'
import pandas as pd
import boto3
import io
...
cat > /home/ubuntu/start_product_matching.sh << 'STARTUP_EOF'
#!/bin/bash
cd /home/ubuntu/price-scanner
...
SETUP_EOF
    ...
    echo "✅ Setup script already exists"
fi
...
# After setup completes, run the product matching
