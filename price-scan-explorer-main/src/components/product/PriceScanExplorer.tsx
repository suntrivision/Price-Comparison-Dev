...
export function PriceScanExplorer() {
  ...
  // Extract products from HTML using JavaScript DOM parsing (similar to BeautifulSoup)
  ...
  // Calculate price data with discount percentages
  const calculatePriceData = (products: Product[]): PriceData[] => {
    return products.map(product => {
      let discountPercentage: number | null = null;
      
      if (product.sale_price && product.original_price) {
        const currentPrice = parseFloat(product.sale_price.replace('฿', ''));
        const originalPrice = parseFloat(product.original_price.replace('฿', '').replace('฿', ''));
      ...
      return {
        ...
        currentPrice: product.sale_price || 'N/A',
        originalPrice: product.original_price?.replace('฿฿', '฿') || null,
        discountPercentage
      };
    });
  };
  ...
  const handleRunScript = async () => {
    ...
    try {
      ...
      // Calculate price data with discounts
      const calculatedPriceData = calculatePriceData(products);
      ...
      toast({
        ...
        description: `Found ${products.length} products with price data`,
        variant: "default",
      });
    } catch (error) {
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isRunning ? "Running..." : "Run Price Scan"}
          </Button>
        </div>
        <p className="text-xs text-muted-foreground">
          Enter a URL containing products to scan for prices. Try websites like Lotus's, Shopee, or Amazon.
        </p>
      </div>
      ...
      {isRunning && (
        <div className="space-y-2">
          <div className="text-sm text-muted-foreground flex justify-between">
            <span>Running price scanner...</span>
            <span>{progress}%</span>
          </div>
      ...
      {showResults && (
        <div className="mt-6 border-t pt-4 border-border">
          <Tabs defaultValue="price-analysis" className="w-full">
            <TabsList>
            ...
            <TabsContent value="price-analysis" className="mt-4">
              <h4 className="font-medium mb-3">Price Scan Results</h4>
              <div className="grid gap-4">
                {priceData.map((item, index) => (
                  <Card key={index} className="p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h5 className="font-medium text-sm line-clamp-2">{item.product}</h5>
